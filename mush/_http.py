...
def host_header(info):
    if (
        ...
        or (not info["tls"] and info["port"] == 80)
    ):
        return info["host"]
    return "{}:{}".format(
        info["host"],
...
def join_url(base, location):
    if location.startswith(
        ...
        return location
    info = parse_url(base)
    scheme = "https" if info["tls"] else "http"
    if location.startswith("/"):
        path = location
    else:
        path = (
...
def request_once(
    ...
    if connect is None:
        raise ValueError("missing connect function")
    info = parse_url(url)
    sock = connect(
        info,
        timeout,
...
def request(
    ...
    while True:
        ...
        if response.code() not in _REDIRECTS:
            return response
        if redirects <= 0:
            response.close()

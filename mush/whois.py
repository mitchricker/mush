...
EXAMPLES
    whois("example.com")
    whois("8.8.8.8")
    whois("example.com",verbose=True)
"""
import mush
...
def _print_notices(notices):
    ...
    for notice in notices:
        ...
        if title:
            print("  {}".format(title))
        for line in notice.get("description", []):
            print("    {}".format(line))
...
def main(query, verbose=False):
    try:
        ...
        if obj:
            print("Class: {}".format(obj))
        _print_list("Status", data.get("status"))
        _print_events(data.get("events"))
        if not verbose:
            return
        _print_nameservers(data.get("nameservers"))
        _print_entities(data.get("entities"))
        _print_notices(data.get("notices"))
        _print_links(data.get("links"))
    except Exception as e:
        print("whois: {}: {}".format(query, e))

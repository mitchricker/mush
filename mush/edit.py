...
class FileBuffer:
    ...
    def save(self):
        def writer(f):
            for line in self.lines:
                f.write((line+"\n").encode())
        fsio.atomic_write(self.path,writer)
        self.dirty=False
    def line_count(self):
    ...
    def insert_char(self,row,col,char):
        s=self.lines[row]
        self.lines[row]=s[:col]+char+s[col:]
        self.dirty=True
    def delete_char(self,row,col):
        s=self.lines[row]
        self.lines[row]=s[:col]+s[col+1:]
        self.dirty=True
def _check_recovery(path):
    ...
    try:
        ...
        return
    try:
        if input("Recovery file found: {}\nRecover? [y/N]: ".format(tmp)).lower()=="y":
            return tmp
    except:
        pass
def _clear():
...
def _move(row,col):
    print("{}[{};{}H".format(ESC,row,col),end="")
def _read_key():
    c=sys.stdin.read(1)
    if c!=ESC:
        return c
    ...
    if seq=="[6":
        ...
        return "PGDN"
    return ""
def _draw(buf,row,col,top):
    _clear()
    for y in range(SCREEN_ROWS):
        n=top+y
        if n<buf.line_count():
            text=buf.get(n)[:SCREEN_COLS]
            print("{:<78}".format(text))
        else:
            print("")
    print("-"*SCREEN_COLS)
    dirty=" [modified]" if buf.dirty else ""
    print("{}{} Ln {}/{} Col {} Ctrl-S save Ctrl-Q quit".format(
        buf.path,
        dirty,
        row+1,
        buf.line_count(),
    ...
    _move(row-top+1,col+1)
def main(path):
    source=_check_recovery(path)
    buf=FileBuffer(path)
    if source:
        print("Recovering:",source)
        buf.load(source)
        buf.dirty=True
    else:
        buf.load()
    ...
    while True:
        ...
        if key=="\x13":
            ...
            if buf.dirty:
                try:
                    if input("Unsaved changes. Quit without saving? [y/N]: ").lower()!="y":
                        continue
                except:
                    continue
            break
            ...
            row=max(0,row-SCREEN_ROWS)
        elif key=="PGDN":
            row=min(buf.line_count()-1,row+SCREEN_ROWS)
        elif key in ("\r","\n"):
            s=buf.get(row)
            buf.lines[row]=s[:col]
            buf.insert_line(row+1,s[col:])
            row+=1
            col=0
        elif key in ("\b","\x7f"):
            if col:
